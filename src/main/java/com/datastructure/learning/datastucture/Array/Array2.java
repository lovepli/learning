...
public class Array2<E> {
  ...
  public void add(int index, E e) { // 理解ppt的图片size的值：数组中第一个没有元素的位置
    ...
    if (index < 0 || index > size) {
      throw new IllegalArgumentException(
          "Add failed,Array is full;Require index >=0 and index<=size");
    }
  ...
  public E remove(int index) {
    ...
    size--;
    // TODO (闲逛的对象)loitering objects !=memory leak  使之置为空，让垃圾回收
    data[size] = null;
    // TODO 删除元素之后，容量小到一定程度，进行缩容 后面采用的是懒缩容，缩容到1/4
  ...
  /** 实现数组的动态扩容和缩容 此方法为private TODO 数组动态扩容 */
  private void resize(int newCapacity) {
    // TODO  新建Object[] 数组  泛型数组要强制转换
  ...
  public static void main(String[] args) {
    ...
    array2.remove(2);
    //  System.out.println((array2.data[array2.size]);
    System.out.println(array2.toString());
  }
   ...
   * 关于时间复杂度的分析
   *
   * <p>增：O(n) 注意：如果只对最后一个元素操作，依然是O(n)？ 因为resize？ 删：O(n) 改：已知索引O(1);未知索引O(n) 查：已知索引O(1);未知索引O(n)
   *
   * <p>均摊复杂度 和防止复杂度的震荡
   */
}
